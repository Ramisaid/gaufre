       ...
       Point test = jPanel1.getLocationOnScreen();
        ...
        for(int i =0; i< COLONNE_GRILLE; i++) { // colone
            for(int j = 0; j< LIGNE_GRILLE; j++) { // ligne
                ...
                if(changemap == true ) {
                    Point tmp = new Point(test.x, test.y);
                    jLabel1.setText(tmp.x+" "+poschange.x+" "+tmp.y+" "+poschange.y);
                    if(tmp.x >= poschange.x && tmp.x <= poschange.x +(uniteX)  && tmp.y >= poschange.y && tmp.y<= poschange.y+(uniteY) ) {
